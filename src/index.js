...
class ChartBig extends React.Component {
  render() {
    ...
    const calculatedData = macdCalculator(smaVolume50(sma200(sma50(sma20(dataToCalculate)))));
    ...
    return (
      <div className='row no-gutters chart-chart bg-lightgray-ultra-5 margin-bottom-10'>
        ...
        >
          <Chart id={1}
            height={height * 0.8}
            yExtents={[d => [d.high, d.low], sma20.accessor(), sma50.accessor(), sma200.accessor()]}
            origin={(w, h) => [0, 0]}
            padding={{ top: 10, bottom: 20 }}
          >
            <XAxis tickStroke={'#cccccc'} axisAt='bottom' orient='bottom' ticks={ticks} {...xGrid} />
